...
/**
 * {@code EqualUserIdException} 类用于表示当不满足用户 ID 唯一性要求时抛出的异常。
 ...
 */
public class EqualUserIdException extends MyException {
    private static final ErrorCount errorCount = new ErrorCount();
    private final int id;

    /**
     * 构造一个新的 {@code EqualUserIdException} 实例。
     *
     * @param id 触发异常的用户 ID
     */
    public EqualUserIdException(int id) {
        this.id = id;
    ...
    /**
     * 输出异常的统计信息到标准输出。
     * 格式为：eui-{@code x}, {@code id}-{@code y}，其中 {@code x} 为此类异常发生的总次数，
     * {@code y} 为该 {@code id} 触发此类异常的次数。
     */
    public void print() {
        System.out.println("eui-" + errorCount.getCount() + ", "
