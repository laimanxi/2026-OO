...
/**
 * {@code DuplicateMedalException} 类用于表示当用户尝试购买已购买的 up 主徽章时抛出的异常。
 * 该异常类包含两个整型参数 {@code id1} 和 {@code id2}，表示触发异常的用户 ID 和 up 主 ID。
 *
 * <p>异常统计信息的格式为：dm-{@code x}, {@code id1}-{@code y}, {@code id2}-{@code z}，
 * 其中 {@code x} 为此类异常发生的总次数，{@code y} 为 {@code id1} 触发此类异常的次数，
 * {@code z} 为 {@code id2} 触发此类异常的次数。</p>
 */
public class DuplicateMedalException extends MyException {
    ...
    /**
     * 构造一个新的 {@code DuplicateMedalException} 实例。
     *
     *
     * @param id1 触发异常的用户 ID
     * @param id2 触发异常的用户 ID
     */
    public DuplicateMedalException(int id1, int id2) {
        this.id1 = id1;
        this.id2 = id2;
        count++;
    ...
    /**
     * 输出异常的统计信息到标准输出。
     * 格式为：dm-{@code x}, {@code id1}-{@code y}, {@code id2}-{@code z}，
     * 其中 {@code x} 为此类异常发生的总次数，{@code y} 为 {@code id1} 触发此类异常的次数，
